...
/* To display the configuration page for the module */
	...
	public function hookHeader($params)
	{
		...
		if(Tools::getValue('step') == 3)
		{
			...
			if(Tools::isSubmit('processCarrier'))
			{
				...
				else
				{
					...
					if ($this->_isAlreadyDefinedOrder($id_cart)){
						...
						$this->updateOrderValues($id_cart, $id_order_detail, $this->delivery_date, $minimal_time, $maximal_time, $cart_date);
					}	
					else{
						$this->addOrderValues($id_cart, $this->delivery_date, $minimal_time, $maximal_time, $cart_date);
					}
				}
			}
			else{
				if(!$this->_isCartDefinedToday($id_cart))
				{
					Tools::redirect('index.php?controller=order&step=2&updateorderdetails=1');
				}
			}
		}
	}
			public function updateOrderValues($id_cart, $id_order_detail, $delivery_date, $minimal_time, $maximal_time, $cart_date){
				Db::getInstance()->execute('
					UPDATE `'._DB_PREFIX_.'datetimecustomizer_order_detail`
					SET `id_cart`= '.(int)$id_cart.', `delivery_date`= '.$delivery_date.', `minimal_time` = '.pSQL($minimal_time).', `maximal_time` = '.pSQL($maximal_time).', `cart_date` = , '.(int)$cart_date.'
					WHERE `id_order_detail` = '.(int)($id_order_detail)
					);
			}

			public function addOrderValues($id_cart, $delivery_date, $minimal_time, $maximal_time, $cart_date){
					Db::getInstance()->execute('
						INSERT INTO `'._DB_PREFIX_.'datetimecustomizer_order_detail`(`id_cart`,`delivery_date`,`minimal_time`, `maximal_time`, `cart_date`)
						VALUES ('.(int)$id_cart.','.$delivery_date.','.pSQL($minimal_time).', '.pSQL($maximal_time).', '.(int)$cart_date.')
						');
			}
			...
			public function _isCartDefinedToday($id_cart){
			
				return (bool)Db::getInstance()->getValue('
					SELECT COUNT(*) FROM `'._DB_PREFIX_.'datetimecustomizer_order_detail` WHERE `id_cart` = '.(int)$id_cart).' AND `cart_date` ='.(int)strtotime(date('d-m-Y'));
			}
